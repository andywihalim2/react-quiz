  ...
  return (
    <div>
      <div className={cssVideoContainer}>
        <video ref={videoRef} className={cssVideo} onTimeUpdate={handleTimeUpdate} src='http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4' />
      </div>

      <button type='button' onClick={onTogglePlayPause}>
        {isPlaying ? 'Pause' : 'Play'}
      </button>
      <button type='button' onClick={onToggleMuteUnmute}>
        {isMuted ? 'Unmute' : 'Mute'}
      </button>
      <input type='range' value={seekValue} min='0' max={duration} step='0.01' onChange={onSeek} />
      <span>
        {onFormatTime(currentTime)} / {onFormatTime(duration)}
      </span>
    </div>
  )
}
